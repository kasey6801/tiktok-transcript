...
export async function save(meta, segments, engineUsed, language) {
  ...
  // chrome.storage is the source of truth for the history list; files on disk
  // are the durable copy. The record is already stored by this point, so nothing
  // about writing files may throw past here and turn a saved transcript into a
  // failed job.
  try {
    const settings = await getSettings();
    if (settings.saveToDownloads) {
      await writeTranscriptToDownloads(record, settings.folderFormats, settings.downloadSubfolder);
    }
  } catch (err) {
    log("downloads", `write failed for ${id}, transcript kept in this browser: ${err?.message ?? err}`);
  }

  return id;
}

// Write everything already stored out to disk, so switching saving on does not
// leave the existing history behind.
export async function writeAllToDownloads() {
  ...
  for (const record of records) {
    ...
    const result = await writeTranscriptToDownloads(record, settings.folderFormats, settings.downloadSubfolder);
...
export async function getSettings() {
  ...
  return {
    ...
    downloadSubfolder: "TikTok Transcripts",
    ...stored,
